...
.modal-overlay {
    ...
    align-items: center; /* 使用align-items来辅助居中 */
    padding: 2rem 1rem; /* 为弹窗容器增加上下边距，确保不被遮挡 */
}
...
.modal-content {
    ...
    max-width: 600px;
    width: 100%; /* 宽度调整为100%以适应父级padding */
    transform: scale(0.9);
    ...
    word-break: break-word;
    /* 修正：设置最大高度并开启滚动 */
    max-height: calc(100vh - 4rem); /* 高度为视口高度减去上下的padding */
    overflow-y: auto;
}
